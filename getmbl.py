...
def getMbl():
	...
	b.open('http://www.mbl.is/leiga/leit/?type=&min_rooms=&max_rooms=&min_price=&max_price=&text_query=')
	root = etree.fromstring(b.response().read().decode('ISO-8859-1'), etree.HTMLParser())
	eignir = root.findall('.//div[@class="rental-itemlist-property clear"]')
	pagecounter = 2
	soup = BeautifulSoup(b.response().read())
	mydivs = soup.findAll("div", { "class" : "rental-itemlist-maininfo" })
	checkLastPage = root.find('.//div[@id="property-meta-lastid"]')
	...
	while(eignir != []):
		...
		for eign in eignir:
			...
			if("fm" in rooms):
				rooms = "?"
				size = rooms.replace("fm", "").strip()
			
			
			results.append([addr, loc, price, size, rooms, link])
			
		url = 'http://www.mbl.is/leiga/leit/?page='+str(pagecounter)+'&type=&min_rooms=&max_rooms=&min_price=&max_price=&text_query='
		b.open(url)
		root = etree.fromstring(b.response().read().decode('ISO-8859-1'), etree.HTMLParser())
		eignir = root.findall('.//div[@class="rental-itemlist-property clear"]')
		pagecounter += 1
